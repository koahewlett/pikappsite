    ...
    <main className="relative min-h-screen overflow-hidden bg-ink bg-rose-grid rose">
      ...
      <section id="gallery" className="mx-auto max-w-7xl px-6 py-16">
        ...
        <div className="masonry mt-8">
          {gallery.map((g) => (
            <div key={g} className="glass group h-56 overflow-hidden rounded-[2rem] p-6 transition hover:scale-[1.02] md:h-72">
        ...
        <div className="glass rounded-[2rem] p-8">
          ...
          <div className="mt-8 grid gap-3 md:grid-cols-4">
            {['Entrepreneurs', 'Executives', 'Athletes', 'Graduate Students'].map((x) => (
              <div className="rounded-2xl bg-white/5 p-4 text-gold" key={x}>{x}</div>
            ))}
          </div>
        </div>
      </section>
