...
create policy "Admins can create own pnm ratings" on public.pnm_ratings for insert to authenticated with check (app_private.is_rush_admin((select auth.uid())) and admin_id = (select auth.uid()));
drop policy if exists "Admins can update own pnm ratings" on public.pnm_ratings;
create policy "Admins can update own pnm ratings" on public.pnm_ratings for update to authenticated using (app_private.is_rush_admin((select auth.uid())) and admin_id = (select auth.uid())) with check (app_private.is_rush_admin((select auth.uid())) and admin_id = (select auth.uid()));
drop policy if exists "Admins can delete pnm ratings" on public.pnm_ratings;
create policy "Admins can delete pnm ratings" on public.pnm_ratings for delete to authenticated using (app_private.is_rush_admin((select auth.uid())));
